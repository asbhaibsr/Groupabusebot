...
TAG_EMOJIS = ["ðŸŽ‰", "âœ¨", "ðŸ’«", "ðŸŒŸ", "ðŸŽˆ", "ðŸŽŠ", "ðŸ”¥", "ðŸ’–", "âš¡ï¸", "ðŸŒˆ"]
TAG_MESSAGES = {}
...
async def handle_incident(client: Client, chat_id, user, reason, original_message: Message, case_type, case_id=None):
    ...
    try:
        details_message_text = (
            f"ðŸš¨ <b>à¤¨à¤¯à¤¾ à¤‰à¤²à¥à¤²à¤‚à¤˜à¤¨ ({case_type.upper()})</b> ðŸš¨\n\n"
            f"<b>ðŸ“ à¤—à¥à¤°à¥à¤ª:</b> {original_message.chat.title} (<code>{chat_id}</code>)\n"
            f"<b>ðŸ‘¤ à¤¯à¥‚à¤œà¤¼à¤°:</b> {user.mention} (<code>{user.id}</code>)\n"
            f"<b>ðŸ“ à¤¯à¥‚à¤œà¤¼à¤°à¤¨à¥‡à¤®:</b> @{user.username if user.username else 'N/A'}\n"
            f"<b>â° à¤¸à¤®à¤¯:</b> {datetime.now().strftime('%Y-%m-%d %H:%M:%S IST')}\n"
            f"<b>ðŸ†” à¤•à¥‡à¤¸ ID:</b> <code>{case_id}</code>\n\n"
            f"<b>âž¡ï¸ à¤•à¤¾à¤°à¤£:</b> {reason}\n"
            f"<b>âž¡ï¸ à¤®à¥‚à¤² à¤®à¥ˆà¤¸à¥‡à¤œ:</b> ||{message_text}||\n"
        )
    ...
    notification_message = (
        f"ðŸš¨ **Group mein Niyam Ulanghan!**\n\n"
        f"âž¡ï¸ <b>User:</b> {user.mention}\n"
        f"âž¡ï¸ <b>Reason:</b> \"{reason} ki wajah se message hata diya gaya haià¥¤\"\n\n"
        f"âž¡ï¸ <b>Case ID:</b> <code>{case_id}</code>"
    )

    keyboard = [
        [
            InlineKeyboardButton("ðŸ‘¤ User Profile", url=f"tg://user?id={user.id}"),
            InlineKeyboardButton("ðŸ”§ Admin Actions", callback_data=f"admin_actions_menu_{user.id}_{chat_id}")
        ],
        [
            InlineKeyboardButton("ðŸ“„ View Case Details", url=case_detail_url)
        ]
    ]
...
async def start(client: Client, message: Message) -> None:
    ...
    if chat.type == 'private':
        welcome_message = (
            f"ðŸ‘‹ <b>Namaste {user.first_name}!</b>\n\n"
...
async def stats(client: Client, message: Message) -> None:
    ...
    stats_message = (
        ...
        f"â€¢ Total Incidents Logged: {total_incidents}\n"
...
async def confirm_broadcast(client: Client, query: CallbackQuery) -> None:
    ...
    if broadcast_msg and broadcast_msg != "waiting_for_message":
        ...
        await query.edit_message_text(f"ðŸ“¢ Broadcast shuru ho raha hai...\n\nTotal targets: {len(target_chats)}")
...
async def cancel_broadcast(client: Client, query: CallbackQuery) -> None:
    ...
    await query.edit_message_text("âŒ Broadcast cancel kar diya gaya hai.")
    ...
    if profanity_filter is not None:
        try:
            if profanity_filter.add_bad_word(word_to_add):
                await message.reply_text(f"âœ… Shabd '`{word_to_add}`' safaltapoorvak jod diya gaya hai\\.", parse_mode='MarkdownV2')
                logger.info(f"Admin {message.from_user.id} added abuse word: {word_to_add}.")
            else:
                await message.reply_text(f"Shabd '`{word_to_add}`' pehle se hi list mein maujood hai\\.", parse_mode='MarkdownV2')
        except Exception as e:
...
async def tag_admins(client: Client, message: Message) -> None:
    ...
    try:
        ...
        tag_message_text = " ".join(f"ðŸ‘‘ {admin}" for admin in tagged_admins)
...
async def tag_stop(client: Client, message: Message) -> None:
    ...
    if chat_id not in TAG_MESSAGES or not TAG_MESSAGES[chat_id]:
        await message.reply_text("à¤•à¥‹à¤ˆ à¤­à¥€ à¤Ÿà¥ˆà¤—à¤¿à¤‚à¤— à¤®à¥ˆà¤¸à¥‡à¤œ à¤¨à¤¹à¥€à¤‚ à¤®à¤¿à¤²à¤¾ à¤œà¤¿à¤¸à¥‡ à¤°à¥‹à¤•à¤¾ à¤œà¤¾ à¤¸à¤•à¥‡à¥¤")
        return
    ...
    try:
        ...
        final_message_text = "**à¤¯à¤¹ à¤Ÿà¥ˆà¤—à¤¿à¤‚à¤— à¤–à¤¤à¥à¤® à¤¹à¥‹ à¤—à¤¯à¤¾ à¤¹à¥ˆà¥¤**"
...
async def check_permissions(client: Client, message: Message):
    ...
    try:
        ...
        message_text = (
            ...
            f"<b>âœ… à¤®à¥ˆà¤¸à¥‡à¤œ à¤­à¥‡à¤œ à¤¸à¤•à¤¤à¤¾ à¤¹à¥ˆ:</b> {perms.can_send_messages}\n"
        )
...
async def handle_all_messages(client: Client, message: Message) -> None:
    ...
    if profanity_filter is not None and profanity_filter.contains_profanity(message_text):
        await handle_incident(client, chat.id, user, "à¤—à¤¾à¤²à¥€-à¤—à¤²à¥Œà¤œ (Profanity)", message, "abuse")
        return

    # Check for URLs directly in the message
    if URL_PATTERN.search(message_text) and not is_sender_admin:
        await handle_incident(client, chat.id, user, "à¤®à¥ˆà¤¸à¥‡à¤œ à¤®à¥‡à¤‚ à¤²à¤¿à¤‚à¤• (Link in Message)", message, "link_in_message")
        return
    ...
    if not is_sender_admin and not await is_biolink_whitelisted(user.id):
        try:
            ...
            if URL_PATTERN.search(user_bio):
                ...
                warning_text = (
                    "**ðŸš¨ Chetavni** ðŸš¨\n\n"
                    f"ðŸ‘¤ **User:** {user.mention} (`{user.id}`)\n"
                    "âŒ **Karan:** Bio mein link mila\n"
                    f"âš ï¸ **Chetavni:** {warn_count}/{warn_limit}\n\n"
                    "**Notice: Kripya apne bio se links hata dein.**"
                )
                
                keyboard = [
                    [
                        InlineKeyboardButton("âŒ Chetavni Cancel", callback_data=f"cancel_warn_{user.id}_{chat.id}"),
                        InlineKeyboardButton("âœ… Bio Link Approve", callback_data=f"approve_bio_{user.id}_{chat.id}")
                    ],
                    [InlineKeyboardButton("ðŸ—‘ï¸ Band Karen", callback_data=f"close_message_{message.id}")]
                ]
                reply_markup = InlineKeyboardMarkup(keyboard)
...
async def handle_edited_messages(client: Client, edited_message: Message) -> None:
    ...
    if not is_sender_admin:
        ...
        notification_message = f"ðŸš¨ {user.mention} ne ek message edit kiya jo delete kar diya gaya hai."
...
async def button_callback_handler(client: Client, query: CallbackQuery) -> None:
    ...
    if query.message.chat.type in ['group', 'supergroup'] and not data.startswith(('help_menu', 'other_bots', 'donate_info', 'back_to_main_menu')):
        if not is_current_group_admin:
            await query.answer("âŒ Aapke paas is action ko karne ki permission nahi hai. Aap group admin nahi hain.", show_alert=True)
    ...
    if data == "help_menu":
        help_text = (
            ...
            "<b>â€¢ Bio Link Detection:</b> Automatically deletes messages from users with links in their bio (Admins are exempt).\n"
            ...
            "<b>Commands:</b>\n"
            "â€¢ `/start`: Bot ko start karein (private aur group mein).\n"
            "â€¢ `/stats`: Bot usage stats dekhein (sirf bot admins ke liye).\n"
            "â€¢ `/broadcast`: Sabhi groups mein message bhejein (sirf bot admins ke liye).\n"
            f"â€¢ `/addabuse <shabd>`: Custom gaali wala shabd filter mein add karein (sirf bot admins ke liye).\n"
            f"â€¢ `/checkperms`: Group mein bot ki permissions jaanchein (sirf group admins ke liye).\n"
            "â€¢ `/tagall <message>`: Sabhi members ko tag karein.\n"
            "â€¢ `/onlinetag <message>`: Online members ko tag karein.\n"
            "â€¢ `/admin <message>`: Sirf group admins ko tag karein.\n"
            "â€¢ `/tagstop`: Saare tagging messages ko delete kar dein."
        )
        ...
        bot_name = bot_info.first_name
        welcome_message = (
            f"ðŸ‘‹ <b>Namaste {query.from_user.first_name}!</b>\n\n"
            f"Mai <b>{bot_name}</b> hun, aapka group moderator bot. "
            f"Mai aapke groups ko saaf suthra rakhne mein madad karta hun, "
            f"gaaliyon wale messages ko delete karta hun aur zaroorat padne par warning bhi deta hun.\n\n"
            f"<b>Mere features:</b>\n"
        ...
        actions_keyboard = [
            [InlineKeyboardButton("ðŸ”‡ Mute (30 min)", callback_data=f"mute_{target_user_id}_{group_chat_id}_30m")],
            [InlineKeyboardButton("ðŸ”‡ Mute (1 hr)", callback_data=f"mute_{target_user_id}_{group_chat_id}_1h")],
            [InlineKeyboardButton("ðŸ”‡ Mute (24 hr)", callback_data=f"mute_{target_user_id}_{group_chat_id}_24h")],
            [InlineKeyboardButton("ðŸš« Ban", callback_data=f"ban_{target_user_id}_{group_chat_id}")],
            [InlineKeyboardButton("ðŸ¦µ Kick", callback_data=f"kick_{target_user_id}_{group_chat_id}")],
            [InlineKeyboardButton("â— Warn", callback_data=f"warn_{target_user_id}_{group_chat_id}")],
        ]
        
        if is_biolink_approved:
            actions_keyboard.append([InlineKeyboardButton("âœ… Approved Bio User", callback_data=f"unapprove_bio_{target_user_id}_{group_chat_id}")])
        else:
            actions_keyboard.append([InlineKeyboardButton("âœï¸ Approved Bio User", callback_data=f"approve_bio_{target_user_id}_{group_chat_id}")])

        actions_keyboard.append([InlineKeyboardButton("â¬…ï¸ Back to Notification", callback_data=f"back_to_notification_{target_user_id}_{group_chat_id}")])
        ...
        keyboard = [
            [
                InlineKeyboardButton("ðŸš« Bio Link Unapprove", callback_data=f"unapprove_bio_{target_user_id}_{chat_id}"),
                InlineKeyboardButton("ðŸ—‘ï¸ Band Karen", callback_data=f"close_message_{query.message.id}")
            ]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)

        await query.edit_message_text(
            f"âœ… {mention} (`{target_user_id}`) ko bio link exceptions list mein add kar diya gaya hai.",
            reply_markup=reply_markup,
            parse_mode='html'
        )
        return
        ...
        keyboard = [
            [
                InlineKeyboardButton("âœ… Bio Link Approve", callback_data=f"approve_bio_{target_user_id}_{chat_id}"),
                InlineKeyboardButton("ðŸ—‘ï¸ Band Karen", callback_data=f"close_message_{query.message.id}")
            ]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
        ...
        await query.edit_message_text(
            f"âŒ {mention} (`{target_user_id}`) ko bio link exceptions list se hata diya gaya hai.",
            reply_markup=reply_markup,
            parse_mode='html'
        )
        return
        ...
        try:
            ...
            await query.answer("âŒ Message delete karne mein error hui.", show_alert=True)
        ...
        try:
            ...
            target_user = await client.get_chat_member(group_chat_id, target_user_id)
            await query.edit_message_text(f"âœ… {target_user.user.mention} ko {duration_str} ke liye mute kar diya gaya hai.", parse_mode='html')
        ...
        try:
            ...
            target_user = await client.get_chat_member(group_chat_id, target_user_id)
            await query.edit_message_text(f"âœ… {target_user.user.mention} ko group se ban kar diya gaya hai.", parse_mode='html')
        ...
        try:
            ...
            target_user = await client.get_chat_member(group_chat_id, target_user_id)
            await query.edit_message_text(f"âœ… {target_user.user.mention} ko group se nikal diya gaya hai.", parse_mode='html')
        ...
        try:
            ...
            warn_message = (
                f"ðŸš¨ **Chetavni**\n\n"
                f"âž¡ï¸ {target_user.user.mention}, aapko group ke niyam todne ke liye chetavni di jaati hai. Please group ke rules follow karein.\n\n"
            ...
            if warn_count >= 3:
                ...
                permanent_mute_message = (
                    f"âŒ **Permanent Mute**\n\n"
                    f"âž¡ï¸ {target_user.user.mention}, aapko 3 warnings mil chuki hain. Isliye aapko group mein permanent mute kar diya gaya hai."
                )
                await client.send_message(chat_id=group_chat_id, text=permanent_mute_message, parse_mode='html')
                await query.edit_message_text(f"âœ… {target_user.user.mention} ko {warn_count} chetavaniyan milne ke baad permanent mute kar diya gaya hai.", parse_mode='html')
                logger.info(f"User {target_user_id} was permanently muted after 3 warnings in chat {group_chat_id}.")
            else:
                await query.edit_message_text(f"âœ… {target_user.user.mention} ko chetavni bhej di gayi hai. Warnings: {warn_count}/3.", parse_mode='html')
        ...
        notification_message = (
            f"ðŸš¨ **Group mein Niyam Ulanghan!**\n\n"
            f"âž¡ï¸ <b>User:</b> {user_obj.user.mention}\n"
            f"âž¡ï¸ <b>Reason:</b> \"{reason} ki wajah se message hata diya gaya haià¥¤\"\n\n"
            f"âž¡ï¸ <b>Case ID:</b> <code>{case_id_value}</code>"
        )

        keyboard = [
            [
                InlineKeyboardButton("ðŸ‘¤ User Profile", url=f"tg://user?id={target_user_id}"),
...
if __name__ == "__main__":
    ...
    async def set_commands():
        await client.set_bot_commands([
            ...
            BotCommand("broadcast", "Sabhi groups/users ko message bhejein."),
            BotCommand("addabuse", "Custom abuse word add karein."),
            ...
            BotCommand("tagstop", "Tagging messages ko delete karein."),
